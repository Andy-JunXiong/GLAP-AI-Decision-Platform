...
WITH event_rollup AS (
    SELECT
        ...
        count_if(
            event_type = 'COMPLETE'
            AND trim(actor) <> ''
            AND lower(trim(actor)) NOT IN ('system', 'automation', 'model')
        ) AS named_complete_actor_count,
...
candidate AS (
    SELECT
        ...
        events.named_complete_actor_count,
        IF(
    ...
    WHERE current.status = 'COMPLETED'
      AND current.approved_by IS NOT NULL
      ...
      AND events.named_complete_actor_count = 1
...
SELECT
    ...
    COALESCE(sum(named_complete_actor_count), 0) AS named_complete_actor_count,
...
do {
    ...
    if ($state -in @("FAILED", "CANCELLED")) {
        throw "Read-only COMPLETE reconciliation query did not complete"
    }
...
if ($state -ne "SUCCEEDED") {
    ...
    throw "Read-only COMPLETE reconciliation query timed out"
}
...
$checks = [ordered]@{
    ...
    "Exactly one COMPLETE event" = $counts.complete_event_count -eq 1
    "COMPLETE actor is a named human" = $counts.named_complete_actor_count -eq 1
    "Assignment matches current state" = $counts.assignment_match_count -eq 1
    "No Outcome exists before continuation" = $counts.outcome_count -eq 0
}
foreach ($entry in $checks.GetEnumerator()) {
    Write-Host "$($entry.Key): $($entry.Value)"
}
if ($checks.Values -contains $false) {
    throw "Action COMPLETE reconciliation failed closed"
}
Write-Host "Action COMPLETE reconciliation passed. Protected identifiers were not printed."
