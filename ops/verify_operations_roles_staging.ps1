...
try {
    ...
    $networkTemporalBoundaryValid = `
        $networkPayload.as_of_date -eq $logicalDate -and `
        $networkPayload.source.execution_mode -eq "OPERATIONAL" -and `
        $networkPayload.source.time_basis -eq "ACTUAL_CALENDAR"
    $networkAggregateRedacted = `
        ($networkPayload | ConvertTo-Json -Depth 10 -Compress) -notmatch 'shipment_id|origin_port|destination_port|s3://|arn:'
    $shipmentPayload = Invoke-ApiJson "$endpoint/v1/shipments?limit=1" $tokens.operator
    $shipmentItems = @($shipmentPayload.items)
    $shipmentTemporalBoundaryValid = `
        $shipmentPayload.as_of_date -eq $logicalDate -and `
        $shipmentPayload.source.execution_mode -eq "OPERATIONAL" -and `
        $shipmentPayload.source.time_basis -eq "ACTUAL_CALENDAR" -and `
        @($shipmentItems | Where-Object { [datetime]$_.metric_date -gt [datetime]$logicalDate }).Count -eq 0
    $shipmentFieldBoundaryValid = `
        ($shipmentPayload | ConvertTo-Json -Depth 10 -Compress) -notmatch 'expected_total_cost|current_total_cost|origin_port|destination_port|s3://|arn:'
    $shipmentPaginationValid = $true
    if ($shipmentPayload.next_token) {
        $secondShipmentPage = Invoke-ApiJson `
            "$endpoint/v1/shipments?limit=1&next_token=$([uri]::EscapeDataString($shipmentPayload.next_token))" `
            $tokens.operator
        $shipmentPaginationValid = `
            @($secondShipmentPage.items).Count -le 1 -and `
            (-not $shipmentItems.Count -or -not @($secondShipmentPage.items).Count -or `
             $shipmentItems[0].shipment_id -ne @($secondShipmentPage.items)[0].shipment_id)
    }
    ...
    $checks = [ordered]@{
        ...
        "viewer network read allowed" = $networkReadStatuses.viewer -eq 200
        "viewer shipment entity denied" = $shipmentReadStatuses.viewer -eq 403
        ...
        "network response contract valid" = $networkPayload.schema_version -eq "operations-api.v1"
        "network temporal boundary valid" = $networkTemporalBoundaryValid
        "network aggregate identifiers redacted" = $networkAggregateRedacted
        "network viewer entity access redacted" = $networkPayload.entity_access -eq $false
        "shipment temporal boundary valid" = $shipmentTemporalBoundaryValid
        "shipment field boundary valid" = $shipmentFieldBoundaryValid
        "shipment pagination valid" = $shipmentPaginationValid
        ...
        "operator network read allowed" = $networkReadStatuses.operator -eq 200
        "operator shipment entity read allowed" = $shipmentReadStatuses.operator -eq 200
        ...
        "approver network read allowed" = $networkReadStatuses.approver -eq 200
        "approver shipment entity read allowed" = $shipmentReadStatuses.approver -eq 200
        ...
        "administrator network read allowed" = $networkReadStatuses.administrator -eq 200
    ...
    Write-Host "Network summary: route_provider_rows=$($networkItems.Count), entity_rows_sampled=$($shipmentItems.Count), entity_access_for_viewer=$($networkPayload.entity_access)"
