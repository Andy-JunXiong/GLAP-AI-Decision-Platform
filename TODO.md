...
- [x] Add and validate an Operations API-specific two-role CloudFormation
  deployment boundary: GitHub OIDC has customer-managed orchestration access
  to one stack and artifact prefix, while a CloudFormation-only execution role
  can update only the existing physical resources. Workflow deploy run
  `31156819949` succeeded without broadening the Stateful Lifecycle policy.
