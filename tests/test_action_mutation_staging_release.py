...
class ActionMutationStagingReleaseTests(unittest.TestCase):
    ...
    def test_release_scripts_preserve_cloudformation_ownership(self):
        ...
        self.assertIn("execution-role=$cloudformationrolearn", prepare)
        self.assertIn("--description $changesetdescription", prepare)
        self.assertIn("description -ne $changesetdescription", prepare)
        self.assertIn("cloudformation delete-change-set", prepare)
        self.assertIn("cloudformation execute-change-set", execute)
        self.assertIn("execution-role=$cloudformationrolearn", execute)
        self.assertIn("description -ne $changesetdescription", execute)
        self.assertIn("$newstack.rolearn -ne $cloudformationrolearn", execute)
        self.assertNotIn("$description.rolearn", combined)
