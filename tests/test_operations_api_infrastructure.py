...
class OperationsApiInfrastructureTests(unittest.TestCase):
    ...
    def test_reliability_and_least_privilege_controls_exist(self):
        ...
        self.assertIn("LIFECYCLE_ACTION_AUDIT_TABLE", template)
        self.assertIn("LIFECYCLE_ACTION_TABLE", template)
    ...
    def test_deployment_is_manual_plan_first(self):
        ...
        self.assertIn("CloudFormationRoleArn", script)
        self.assertIn("--role-arn $CloudFormationRoleArn", script)
        self.assertIn("Dedicated CloudFormation execution role configured: True", script)
        ...
        self.assertIn("Action evidence governance boundary valid", role_verifier)
        self.assertIn("Action evidence Outcome boundary valid", role_verifier)
        ...
        self.assertIn("fact_lifecycle_action_staging_v1", workflow)
        self.assertIn("fact_lifecycle_action_audit_staging_v1", workflow)
    ...
    def test_operations_api_deployer_is_two_role_update_only_and_plan_first(self):
        ...
        self.assertIn("GitHub orchestration policy is customer-managed: True", script)
        self.assertIn("GLAPOperationsApiStackExecution", script)
        self.assertIn("glap-operations-api-cloudformation-staging-role", script)
        self.assertIn('"cloudformation:CreateChangeSet"', script)
        self.assertIn('"cloudformation:ExecuteChangeSet"', script)
        self.assertIn('"iam:PassRole"', script)
        self.assertIn('"iam:PassedToService" = "cloudformation.amazonaws.com"', script)
        self.assertIn('"iam:PassedToService" = "lambda.amazonaws.com"', script)
        self.assertIn('"lambda:UpdateFunctionCode"', script)
        self.assertIn('"lambda:GetRuntimeManagementConfig"', script)
        self.assertIn('"iam:PutRolePolicy"', script)
        self.assertIn('"iam:ListAttachedRolePolicies"', script)
        self.assertIn('"apigateway:PATCH"', script)
        self.assertIn('"apigateway:POST"', script)
        self.assertIn('"logs:DescribeLogGroups"', script)
        ...
        self.assertIn('@("iam", "list-policies", "--scope", "Local")', script)
        self.assertIn("iam create-policy --policy-name $GitHubPolicyName", script)
        self.assertIn("iam create-policy-version --policy-arn $githubPolicyArn", script)
        self.assertIn("iam attach-role-policy --role-name $GitHubRoleName", script)
        self.assertIn("iam delete-policy-version --policy-arn $githubPolicyArn", script)
    ...
    def test_discovery_bootstrap_is_read_only_and_plan_first(self):
        ...
        self.assertIn("fact_lifecycle_action_staging_v1", script)
        self.assertIn("fact_lifecycle_action_audit_staging_v1", script)
        self.assertIn("fact_lifecycle_outcome_staging_v1", script)
        self.assertIn("fact_policy_proposal_staging_v1", script)
